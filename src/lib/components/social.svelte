...
<div
    style="--translate: {hoverPos
        ? 2 >= hoverPos && hoverPos >= -2
            ? 3 / hoverPos
            : 0
        : 0}rem; --scale: {hoverPos == 0
        ? 180
        : (5 - Math.abs(hoverPos) + 1) * 20}%;"
>
    <p
        class="icon"
        style="opacity:{(5 - Math.abs(hoverPos)) *
            20}%;transform: scale({hoverPos === 0 && clicked ? 70 : 100}%)"
    >
        <SocialIcon {name} />
    </p>
    <p
        class="name"
        style="opacity:{hoverPos === 0 ? 100 : 0}%;--y:{hoverPos === 0
            ? 2.5
            : 1}rem;--scale:{hoverPos === 0 ? 80 : 50}%"
    >
        {name}&#8203;
    </p>
</div>
